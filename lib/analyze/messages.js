...
module.exports.analyze = function(users, file_list) {
	...
	var computeTotals = function() {
   	...
   	return new Promise(function(fulfill, reject) {
		...
		stats.addStatGroup('reaction_thank_you_received');
		stats.addStatGroup('youtuber');
		stats.addStatGroup('image_poster');
		stats.addStatGroup('reaction_laugh_machine');
		stats.addStatGroup('reaction_golden_sensor_received');
		stats.addStatGroup('mentions');
		stats.addStatGroup('editors');
		stats.addStatGroup('late_night_posters');
		stats.addStatGroup('sick_users');
		stats.addStatGroup('channel_volume');
		stats.addStatCollection('reaction_map');
		stats.addDateCollection('reaction_history');
	    ...
	    file_list.forEach(function(file,index) {
			...
			if( filename !== 'integration_logs.json' &&
				...
				var channel_name = file_parts[file_parts.length-2];
					...
					if( --file_count === 0 ) {
				  	...
				  	obj.forEach(function(item) {
				    	...
				    	var handle = users.getHandle(item.user);
					  	switch( item.type ) {
					  		case 'message':
					  		    // tracking channel posters is a special beast
					  		    //channel_stats(channel_name,item);
					  		    stats.incrementStatGroup('channel_volume',channel_name,1);
...
function reaction_stats(users,item) {
	...
	var d = new Date(0);
	d.setUTCSeconds(item.ts);
	
	var reaction_count = 0;
	item.reactions.forEach(function(r) {
		// track when emojis were used for the first time
		stats.syncDateCollection('reaction_history',r.name,d);
		...
		r.users.forEach(function(u) {
			...
			// count the number of times the user uses each emoji
			stats.incrementStatCollection('reactions_by_user',giver_handle,r.name,1);
		});
		...
		if( r.name === 'pineapple' ) {
			stats.incrementStatGroup('reaction_thank_you_received',item.user,r.count);
		}

		// who received the most golden sensors?
		if( r.name === 'golden_sensor' ) {
			stats.incrementStatGroup('reaction_golden_sensor_received',item.user,1);
		}
		...
		if( _.contains(laugh_emojis,r.name) ) {
			stats.incrementStatGroup('reaction_laugh_machine',item.user,r.count);
		}

	});
	stats.popularMessages(item.text,reaction_count);


}

var channel_statistics = {};
function channel_stats(channel_name,item) {
	if( !channel_statistics.hasOwnProperty(channel_name) ) {
		channel_statistics[channel_name] = {
			message_count : 1,
			posters : {}
		}
	} else {
		channel_statistics[channel_name].message_count++;
	}
}